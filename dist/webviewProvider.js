...
exports.AcademicLabWebviewProvider = void 0;
const vscode = __importStar(require("vscode"));
const notebookBuilder_1 = require("./notebookBuilder");
const openaiIntegration_1 = require("./openaiIntegration");
const projectManager_1 = require("./projectManager");
const config_1 = require("./utils/config");
const logger_1 = require("./utils/logger");
class AcademicLabWebviewProvider {
    constructor(context) {
        this.context = context;
        this.output = (0, logger_1.createOutputChannel)();
    }
    reveal() {
        if (this.view) {
            this.view.show?.(true);
    ...
    resolveWebviewView(webviewView) {
        ...
        webviewView.webview.onDidReceiveMessage(async (message) => {
            if (message.command === 'openSettings') {
                await vscode.commands.executeCommand('workbench.action.openSettings', 'academicLabAdvisor');
                return;
            }
            ...
            try {
                const notebookData = await (0, openaiIntegration_1.generateNotebookFromAnalysis)({
                    problem: message.problem,
                    outcome: message.outcome,
                    data: message.data
                }, this.context.extensionUri, config);
                const notebook = (0, notebookBuilder_1.buildJupyterNotebook)(notebookData, message.problem);
                const projectPath = await (0, projectManager_1.createProject)(message.problem, notebook);
...
exports.AcademicLabWebviewProvider = AcademicLabWebviewProvider;
